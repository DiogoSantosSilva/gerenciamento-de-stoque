...
class Product(db.Model):
    ...
    def get_product_by_id(self):
        try:
            response = db.session.query(Product).filter(Product.id == self.id).first()
        except Exception as error:
            print(error)
            response = None
        finally:
    ...
    def get_total_products(self):
        try:
            response = db.session.query(func.count(Product.id)).first()
        except Exception as error:
            print(error)
            response = []
        finally:
    ...
    def get_last_products(self):
        try:
            response = db.session.query(Product).order_by(Product.date_created).limit(5).all()
        except Exception as error:
            print(error)
            response = []
        finally:
